...
bool ReplaceChars(const string16& input,
                  ...
                  string16* output) {
  return xReplaceChars(input, replace_chars.as_string(), replace_with, output);
}
...
bool RemoveChars(const string16& input,
                 ...
                 string16* output) {
  return xReplaceChars(input, remove_chars.as_string(), string16(), output);
}

bool RemoveChars(const std::string& input,
                 const base::StringPiece& remove_chars,
                 std::string* output) {
  return xReplaceChars(input, remove_chars.as_string(), std::string(), output);
}

bool TrimString(const string16& input,
                const base::StringPiece16& trim_chars,
                string16* output) {
  return xTrimString(input, trim_chars.as_string(), TRIM_ALL, output) !=
      TRIM_NONE;
}
...
TrimPositions TrimWhitespace(const string16& input,
                             ...
                             string16* output) {
  return xTrimString(input, base::string16(kWhitespaceUTF16), positions,
                     output);
}
...
TrimPositions TrimWhitespaceASCII(const std::string& input,
                                  TrimPositions positions,
                                  std::string* output) {
  return xTrimString(input, std::string(kWhitespaceASCII), positions, output);
}
